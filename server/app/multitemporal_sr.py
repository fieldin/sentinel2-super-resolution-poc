"""
Multi-Temporal-like Multi-Temporal Super-Resolution Pipeline
...
def search_scenes(
    ...
    for item in items[:max_scenes]:
        ...
        scenes.append({
            ...
            "assets": {
                "visual": item.assets.get("visual", {}).href if "visual" in item.assets else None,
...
def download_and_mask_scene(
    ...
    if scl_url:
        ...
        cmd = [
            ...
            "-crop_to_cutline",
            "-t_srs", "EPSG:4326",
            "-ts", str(img.shape[2]), str(img.shape[1]),
            "-r", "nearest",
            "-co", "COMPRESS=LZW",
            f"/vsicurl/{scl_url}",
            str(scl_file),
...
def align_scenes_subpixel(
    ...
    for i, (img, mask, info) in enumerate(scenes):
        ...
        try:
            ...
            cross_power = (f_ref * np.conj(f_img)) / (np.abs(f_ref * np.conj(f_img)) + 1e-10)
            shift_map = np.abs(np.fft.ifft2(cross_power))

            max_loc = np.unravel_index(np.argmax(shift_map), shift_map.shape)

            dy = max_loc[0] if max_loc[0] < shift_map.shape[0] // 2 else max_loc[0] - shift_map.shape[0]
            dx = max_loc[1] if max_loc[1] < shift_map.shape[1] // 2 else max_loc[1] - shift_map.shape[1]
            ...
            if abs(dx) > 0.1 or abs(dy) > 0.1:
                ...
                for c in range(img.shape[0]):
                    aligned_img[c] = cv2.warpAffine(
                        img[c], M, (img.shape[2], img.shape[1]),
                ...
                aligned_mask = cv2.warpAffine(
                    mask.astype(np.uint8), M, (mask.shape[1], mask.shape[0]),
...
def apply_sr_to_fused(
    ...
    try:
        ...
        h, w = img_hwc.shape[:2]
        result = cv2.resize(img_hwc, (w * scale, h * scale), interpolation=cv2.INTER_CUBIC)
        result = np.transpose(result, (2, 0, 1)).astype(np.float32) / 255.0

        return result


def multitemporal_pipeline(
    geometry: dict,
    output_dir: Path,
    days_back: int = 30,
    ...
    """Complete Multi-Temporal-like pipeline."""
    print("=" * 60)
    print("🛰️ Multi-Temporal-like Multi-Temporal SR Pipeline")
    print("=" * 60)
    print(f"   Target: ~70-85% of Multi-Temporal quality")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Multi-Temporal-like Multi-Temporal SR Pipeline")
    parser.add_argument("-p", "--polygon", required=True, help="GeoJSON file or string")
    parser.add_argument("-o", "--output", default="./multitemporal_output", help="Output directory")
